...
def get_index_path(filename: str, model_name: str, processor_name: str, mode: str) -> str:
    """
    根据输入路径和模型名称生成索引的保存路径。
    """
    return f"./tmp/{filename}-index-{model_name.replace('/', '_')}-{processor_name}.{mode}.index"